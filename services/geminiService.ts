...
export const refineUI = async (currentCode: string, prompt: string): Promise<string> => {
  if (!process.env.API_KEY) {
    throw new Error("API_KEY environment variable not set");
  }
  ...
  try {
    ...
    const fullPrompt = `
      ...
      CONTEXT:
      The user wants to modify an existing webpage.
      ...
      INSTRUCTIONS:
      - Return the FULLY UPDATED HTML code. Do not return partial snippets.
      - Maintain the existing design and structure unless explicitly asked to change it.
      - Ensure Tailwind CSS and Lucide icons remain correctly integrated.
    `;
    ...
    console.error("Error refining UI with Gemini:", error);
